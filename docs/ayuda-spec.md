...
| ~~Las variables `REPORTES_V2_*` de `.env.example` **no las lee nadie**: el código usa los mismos nombres sin el `V2_`~~ **ARREGLADO 2026-08-21**: renombradas en `.env.example` y en el `.env` local. **Producción ya usaba los nombres correctos**: solo tenía un `REPORTES_V2_STORAGE_PATH` duplicado, que se sacó del compose. Ojo que ahí no se definen `SYNC_THRESHOLD`, `CHUNK_SIZE` ni `HARD_LIMIT`: corren con los defaults del código (5000 / 1000 / 200000) | `.env.example`, `docker-compose.prod.yml` |
...
| ~~🔴 **La clave de Neotel queda en claro en la auditoría.**~~ **ARREGLADO 2026-08-21**: se agregó `claveNeotel` a los campos sensibles, y `sipPassword` al sanitizador de logs, que hace match exacto y tampoco la cubría. **Verificado en producción: 0 filas de `transaccion` con la clave en claro**, aunque hay 1 agente cargado — nunca se dio de alta por la API. Nada que limpiar | `audit.enums.ts`, `sanitize.ts` |
