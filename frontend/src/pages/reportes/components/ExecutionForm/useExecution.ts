...
import { reportesApi, Estimacion } from '../../../../api/reportes'
...
type UseExecutionReturn = {
  ...
  estimar: (plantillaId: number, filtrosVars: Record<string, any>) => Promise<Estimacion | null>
...
const useExecution = (): UseExecutionReturn => {
  ...
  const estimar = async (
    ...
    try {
      const res = await reportesApi.estimar(plantillaId, filtrosVars)
  ...
  const execute = async (plantillaId: number, filtrosVars: Record<string, any>) => {
    ...
    try {
      const response = await api.post(
        `/reportes/plantillas/${plantillaId}/ejecutar`,
        { filtrosVars },
        { responseType: 'blob' },
      )
      ...
      if (response.status === 202) {
        ...
        setExecuting(false)
        setTimeout(() => navigate('/reportes/ejecuciones'), 800)
        return
      }
