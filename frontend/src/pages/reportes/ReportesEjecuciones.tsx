...
import { useReportesSocket } from './hooks/useReportesSocket'
...
const estadoToStatus: Record<EstadoEjecucion, StatusValue> = {
  PENDIENTE: 'pending',
  EJECUTANDO: 'running',
...
const ReportesEjecuciones = () => {
  const notify = useNotify()
  const confirm = useConfirm()
  const [items, setItems] = useState<Ejecucion[]>([])
  ...
  const cargar = useCallback(async () => {
    setLoading(true)
    try {
      const res = await reportesApi.listarEjecuciones({
        page,
        pageSize,
  ...
  const handleDescargar = async (item: Ejecucion) => {
    try {
      const res = await reportesApi.descargarEjecucion(item.id)
  ...
  const handleCancelar = async (id: number) => {
    try {
      await reportesApi.cancelarEjecucion(id)
  ...
  const handleEliminar = async (id: number) => {
    ...
    if (!ok) return
    try {
      await reportesApi.eliminarEjecucion(id)
  ...
  const columns: DataTableColumn<Ejecucion>[] = [
    {
      key: 'id',
      label: 'ID',
      ...
      <SectionCard sx={{ mb: 2 }}>
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
          <TextField
            ...
            value={estado}
            onChange={(e) => {
              setEstado(e.target.value as EstadoEjecucion | '')
              setPage(1)
            }}
            size="small"
            sx={{ minWidth: 200 }}
...
export default ReportesEjecuciones
