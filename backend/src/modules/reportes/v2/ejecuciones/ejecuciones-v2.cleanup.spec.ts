...
import { EjecucionesV2CleanupService } from './ejecuciones-v2.cleanup';
import { PrismaService } from '../../../../prisma/prisma.service';
import { ReportesStorageService } from '../storage/reportes-storage.service';

describe('EjecucionesV2CleanupService', () => {
  let svc: EjecucionesV2CleanupService;
  let prismaMock: any;
  let storageMock: any;
  ...
  beforeEach(async () => {
    ...
    svc = module.get(EjecucionesV2CleanupService);
  });

  it('elimina ejecuciones FINALIZADA/FALLIDA antiguas y sus archivos', async () => {
    prismaMock.ejecucion_reporte_v2.findMany.mockResolvedValue([
    ...
    expect(prismaMock.ejecucion_reporte_v2.delete).toHaveBeenCalledTimes(3);

    const filter = prismaMock.ejecucion_reporte_v2.findMany.mock.calls[0][0]
      .where;
  ...
  it('continúa si delete de una falla', async () => {
    ...
    prismaMock.ejecucion_reporte_v2.delete
  ...
  it('respeta días configurados via env', async () => {
    process.env.REPORTES_V2_RETENTION_DAYS = '7';
    prismaMock.ejecucion_reporte_v2.findMany.mockResolvedValue([]);
    ...
    delete process.env.REPORTES_V2_RETENTION_DAYS;
  });
});
