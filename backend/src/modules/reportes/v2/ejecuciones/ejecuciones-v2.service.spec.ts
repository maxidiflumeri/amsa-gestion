...
import { EjecucionesV2Service } from './ejecuciones-v2.service';
...
describe('EjecucionesV2Service', () => {
  let service: EjecucionesV2Service;
  let prismaMock: any;
  let storageMock: any;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getQueueToken(REPORTES_V2_QUEUE_NAME),
          useValue: queueMock,
        },
      ],
    }).compile();

    service = module.get(EjecucionesV2Service);
  });

  describe('estimar', () => {
    it('lanza NotFound si plantilla no existe', async () => {
      prismaMock.plantilla_reporte_v2.findUnique.mockResolvedValue(null);
      await expect(service.estimar(99)).rejects.toThrow(NotFoundException);
    });

    it('sugiere sync si total <= umbral', async () => {
      prismaMock.plantilla_reporte_v2.findUnique.mockResolvedValue({
        id: 1,
        definicion: { columnas: [] },
        raiz: 'deudor',
      });
  ...
  describe('encolarEjecucion', () => {
    it('crea ejecucion PENDIENTE y agrega job a la queue', async () => {
      ...
      prismaMock.ejecucion_reporte_v2.create.mockResolvedValue({ id: 555 });
      prismaMock.ejecucion_reporte_v2.update.mockResolvedValue({});
  ...
  describe('cancelar', () => {
    it('rechaza si estado no es cancelable', async () => {
      prismaMock.ejecucion_reporte_v2.findUnique.mockResolvedValue({
        id: 1,
        usuarioId: 42,
    ...
    it('marca cancelado=true y remueve job en estado waiting', async () => {
      ...
      expect(prismaMock.ejecucion_reporte_v2.update).toHaveBeenCalledWith(
  ...
  describe('eliminar', () => {
    ...
    it('elimina archivo y registro si está finalizada', async () => {
      ...
      prismaMock.ejecucion_reporte_v2.delete.mockResolvedValue({});

      const r = await service.eliminar(1, 42);
      expect(r.ok).toBe(true);
      expect(storageMock.eliminarArchivo).toHaveBeenCalledWith('/tmp/x.xlsx');
      expect(prismaMock.ejecucion_reporte_v2.delete).toHaveBeenCalled();
    });
  });
