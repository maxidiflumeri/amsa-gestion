...
describe('ReportesStorageService', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'reportes-test-'));
    process.env.REPORTES_STORAGE_PATH = tmpRoot;
    svc = new ReportesStorageService();
  });

  afterEach(() => {
    delete process.env.REPORTES_STORAGE_PATH;
  ...
  it('crea estructura anio/mes', async () => {
    ...
    expect(result.filePath).toContain(path.join('reportes', anio, mes));
  });
